...
def generar_reporte_pdf(df_f, anios_sel, meses_sel, suc_sel):
    ...
    # --- PROCESOS ESTRATEGICOS (2 tarjetas) ---
    alto = H_HEAD + PAD + H_CARD + PAD
    marco(y, alto)
    cabecera_proceso(y, "PROCESOS ESTRATÉGICOS", RGB_NAVY)
    w = w_col(2)
    yy = y + H_HEAD + PAD
    pill(X0, yy, w, H_CARD, "010101 | Gerencia General", _monto_ceco("Gerencia General"), RGB_NAVY)
    pill(X0 + w + GAP, yy, w, H_CARD, "010102 | Nuevos Negocios", _monto_ceco("Nuevos Negocios"), RGB_NAVY)
    y += alto + G_CONT

    # --- PROCESOS OPERATIVOS (4 columnas + Marketing) ---
    columnas_op = [
    ...
    H_COLH = 5.5
    H_BAN = 5.5
    max_cards = max(len(c[1]) for c in columnas_op)
    alto_cols = H_COLH + 2 + max_cards * H_CARD + (max_cards - 1) * G_V
    alto = H_HEAD + PAD + alto_cols + 2.5 + H_BAN + G_V + H_CARD + PAD
    marco(y, alto)
    ...
    pdf.rect(X0, ym, W_IN, H_BAN, style="F")
    texto_ajustado(X0, ym + 0.7, W_IN, 4, "MARKETING", 8, "B", "C", (255, 255, 255))
    w_mk = 90
    pill(MARGEN + (ANCHO_UTIL - w_mk) / 2, ym + H_BAN + G_V, w_mk, H_CARD,
         "Marketing-Gastos Generales", _monto_ceco("Marketing-Gastos Generales"))
    y += alto + G_CONT

    # --- PROCESOS DE SOPORTE (4 + 4 + 2 tarjetas) ---
    filas_soporte = [
    ...
    ramas = [
        dict(
            ...
            monto=_monto_area("Gerencia General", "Nuevos Negocios"),
            hijos=[
                ("Gerencia General", _monto_area("Gerencia General")),
                ("Nuevos Negocios", _monto_area("Nuevos Negocios")),
            ],
        ),
        dict(
...
def generar_reporte_pptx(df_f, anios_sel, meses_sel, suc_sel):
    ...
    def pill(s, x, y, w, h, titulo, valor, borde=None, sz_t=11.5, sz_v=15.5):
        ...
        rect(s, x, y, 0.08, h, borde or AZUL)
    ...
    w = wcol(2)
    pill(s, X0, y + HH + PAD, w, HC, "010101 | Gerencia General", _monto_ceco(df_f, "Gerencia General"), NAVY)
    pill(s, X0 + w + GAP, y + HH + PAD, w, HC, "010102 | Nuevos Negocios", _monto_ceco(df_f, "Nuevos Negocios"), NAVY)
    y += alto + 0.1
    ...
    HCOL, HBAN = 0.28, 0.26
    max_cards = max(len(c[1]) for c in columnas_op)
    alto_cols = HCOL + 0.06 + max_cards * HC + (max_cards - 1) * GV
    alto = HH + PAD + alto_cols + 0.14 + HBAN + 0.05 + HC + PAD
    ...
    for i, (nom, cecos) in enumerate(columnas_op):
        ...
        for ceco in cecos:
            ...
            yc += HC + GV
    ym = yy + alto_cols + 0.14
    ban = rect(s, X0, ym, WIN, HBAN, C("#334155"))
    poner_texto(ban, [("MARKETING", 11.5, True, BLANCO)], PP_ALIGN.CENTER, m=(0.05, 0, 0.05, 0))
    wmk = 4.2
    pill(s, MX + (W_UTIL - wmk) / 2, ym + HBAN + 0.05, wmk, HC,
    ...
    ramas = [
        ("GERENCIA ESTRATÉGICA", "Planeación & Expansión", False,
         _monto_area(df_f, "Gerencia General", "Nuevos Negocios"),
         [("Gerencia General", _monto_area(df_f, "Gerencia General")),
          ("Nuevos Negocios", _monto_area(df_f, "Nuevos Negocios"))]),
    ...
    if len(df_suc):
        barras(s, 6.75, 1.3, SW - MX - 6.75, 5.7, list(df_suc["Sucursal"]), list(df_suc["Saldo"]),
               "Gasto por sucursal (S/)", AZUL, size=13)
    ...
    if len(df_mot):
        ...
        barras(s, MX, 1.3, mitad, 5.7, list(df_mot["Motivo_Alerta"]), list(df_mot["sum"]),
               "Monto por motivo (S/)", AZUL, size=12)
